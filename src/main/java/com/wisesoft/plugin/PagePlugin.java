...
/**
 * Mybatis的分页查询插件，通过拦截StatementHandler的prepare方法来实现。 只有在参数列表中包括Page类型的参数时才进行分页查询。
 * 在多参数的情况下，只对第一个Page类型的参数生效。 另外，在参数列表中，Page类型的参数无需用@Param来标注
 * @author wangyong 2015-03-20
...
public class PagePlugin implements Interceptor {
	...
	/**
	 * select id, articleNo, sum(ddd) ss, articleName, (SELECT loginName from sys_userinfo u where
	 * u.id=userId) loginName, (SELECT userName from sys_userinfo u where u.id=userId) userName,
	 * sum(ddd) ss from article 兼容以上子查询 //去除sql ..from 前面的字符。考虑 aafrom fromdd 等等情况
	 */
